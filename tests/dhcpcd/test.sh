...
_build() {
	...
	make clean
	names[2]="build kqueue" ; CC=$compiler LD=$compiler make -j`nproc` |& tee rv_build_2.txt ; results[2]="$?" ; postup 2
	...
	make clean
	names[3]="build pollts" ; CC=$compiler LD=$compiler make -j`nproc` |& tee rv_build_3.txt ; results[3]="$?" ; postup 3
	...
	make clean
	names[4]="build pselect" ; CC=$compiler LD=$compiler make -j`nproc` |& tee rv_build_4.txt ; results[4]="$?" ; postup 4
	...
	make clean
	names[5]="build epoll" ; CC=$compiler LD=$compiler make -j`nproc` |& tee rv_build_5.txt ; results[5]="$?" ; postup 5
	...
	make clean
	names[6]="build ppoll" ; CC=$compiler LD=$compiler make -j`nproc` |& tee rv_build_6.txt ; results[6]="$?" ; postup 6
...
_test() {
	...
	names[1]="eloop-bench-epoll" ; ./eloop-bench-epoll -a 2 -r 50 |& tee rv_out_1.txt ; results[1]="$?" ; process_config 1

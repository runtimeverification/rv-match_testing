...
_build() {
    ...
    sed -i -e 's/all install_docs install_sw/all install_sw/' pkgs/libs/ssl/src/Makefile.org |& tee rv_build_0.txt ; results[0]="$?" ; postup 0

    names[1]="sed out gsl docs"
    sed -i -e 's/bspline doc/bspline/'                        pkgs/libs/gsl/src/Makefile.in  |& tee rv_build_1.txt ; results[1]="$?" ; postup 1
    ...
    sed '100,106{s/^/\/\//}' -i pkgs/libs/uptcpip/src/include/sys/bsd__types.h |& tee rv_build_2.txt ; results[2]="$?" ; postup 2

    names[3]="parsecmgmt"
    bin/parsecmgmt -a build |& tee rv_build_3.txt ; results[3]="$?" ; postup 3
}

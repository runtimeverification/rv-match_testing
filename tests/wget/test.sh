...
_build() {
    cd wget/
    ./configure --disable-threads CC=$compiler LD=$compiler |& tee kcc_build_0.txt ; results[0]="$?" ; postup 0
    bash $base_dir/timeout.sh -t 2000 make |& tee kcc_build_1.txt ; results[1]="$?" ; postup 1
}

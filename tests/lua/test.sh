...
_build() {
    ...
    names[1]="make linux" ; sudo make -j`nproc` linux CC=$compiler LD=$compiler $reportflag |& tee rv_build_1.txt ; results[1]="$?" ; postup 1
}

_test() {
    cd lua-$VERSION-tests/
    names[0]="lua all" ; $build_dir/lua-$VERSION/src/lua all.lua |& tee rv_out_0.txt ; results[0]="$?" ; process_config 0
}

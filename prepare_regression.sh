#/bin/bash
# This file is a special copy of prepare.sh to be used specifically for regression tests.
...
process_kcc_config() {
    ...
    #else
        #echo "prepare_regression.sh did not find a kcc_config in "$(dirname $(pwd))
    fi
    cd $build_dir
}
...
process_config() {
    ...
    #else
        #echo "prepare_regression.sh did not find a config in "$(dirname $(pwd))
    fi
    cd $build_dir
}
...
prep_build() {
    ...
    if [ ! -e $build_dir/build_function_hash ] || [ "$(echo $(sha1sum <<< $buildhashinfo))" != "$(head -n 1 $build_dir/build_function_hash)" ] || [ "0" == "0" ] ; then
        ...
        echo $report_string" building. All projects for regression testing are currently set to always build, meaning to not skip the build process even when nothing has changed."
...
prep_extract_test() {
    ...
    if [[ -v results[@] ]] ; then
        for t in "${!results[@]}"
        do
            echo '<testcase classname="regression.'${test_name/./"_"}'" name="'$compiler' '${names[$t]}'">' >> $report_file
            if [[ ${results[$t]} == 0 ]] ; then
